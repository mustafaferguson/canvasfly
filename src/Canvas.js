 ...
 * created and maintained by KFCarth.
 */
class canvasfly {
    constructor() {
    ...
    static async color(color = "RANDOM") {
        ...
        const ctx = canvas.getContext("2d");
        ctx.fillStyle = canvasfly._getHex(color);
    ...
    static async wasted(Image) {
        let converted = await canvasfly.greyscale(Image);
     ...
     * @returns {Promise<Buffer>}
     * @example let img = await canvasfly.distracted("firstImage.png", "secondImage.png", "thirdImage.png")
     * canvasfly.write(img, "distracted.png");
     */
    static async distracted(image1, image2, image3 = null) {
        ...
        image1 = await jimp.read(await canvasfly.circle(image1));
        image1.resize(150, 150);

        image2 = await jimp.read(await canvasfly.circle(image2));
        image2.resize(130, 130);

        base.composite(image1, 180, 90);
        base.composite(image2, 480, 35);
        if (image3 && image3 !== null) {
            image3 = await jimp.read(await canvasfly.circle(image3));
     ...
     * @returns {Promise<Buffer>}
     * @example const image = await canvasfly.phub({ username: "User", message: "Looks good 👏🏻", image: "..." });
     */
     ...
     * @returns {Promise<Buffer>}
     * @example const img = await canvasfly.spotify({
     *     title: "Faded",
     *     artist: "Alan Walker",
     ...
     * });
     *
     * canvasfly.write(img, "spotify.png");
     */
     ...
     * @example const data = await canvasfly.color("blue");
     * const img = canvasfly.Attachment(data, "color.png");
     ...
     * @example const img = await canvasfly.rotate(image, 90);
     * canvasfly.write(img, "image.png");
     * @returns {Promise<Buffer>}
     */
